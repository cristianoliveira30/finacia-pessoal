<x-layouts.app :title="__('Agenda do Prefeito')">
        ...
        <div class="max-w-7xl mx-auto space-y-6">
            ...
            <div
                ...
                <div class="flex gap-3 mt-4 md:mt-0">
                    {{-- NOVO: botão Mês --}}
                    <button id="btn-agenda-month" type="button" class="px-4 py-2 bg-white hover:bg-slate-50 dark:bg-slate-900 dark:hover:bg-slate-700 black:bg-zinc-950 black:hover:bg-zinc-800
                    ...
                    <div class="lg:col-span-1 space-y-6">
                        <div
                            ...
                            <div id="mini-calendar-grid" class="grid grid-cols-7 gap-2 text-center text-xs">
                                ...
                                @for ($i = 1; $i <= 31; $i++) <button type="button" data-mini-day="{{ $i }}"
                ...
                <div
                    ...
                    <div class="flex items-center justify-between gap-3 mb-4">
                        ...
                        <div class="flex items-center gap-2">
                            <button id="btn-month-prev" type="button" class="w-10 h-10 rounded-lg border border-slate-200 dark:border-slate-700 black:border-zinc-800
                            ...
                            <button id="btn-month-next" type="button" class="w-10 h-10 rounded-lg border border-slate-200 dark:border-slate-700 black:border-zinc-800
                                       bg-white dark:bg-slate-900 black:bg-zinc-950 hover:bg-slate-50 dark:hover:bg-slate-700 black:hover:bg-zinc-800
                                       text-slate-700 dark:text-white black:text-zinc-100 font-bold">
                                ›
                            </button>
                            ...
                            <button id="btn-agenda-day" type="button" class="px-4 py-2 bg-white hover:bg-slate-50 dark:bg-slate-900 dark:hover:bg-slate-700 black:bg-zinc-950 black:hover:bg-zinc-800
                                       text-slate-700 dark:text-white black:text-zinc-100 rounded-lg text-sm font-bold transition-colors
                                       border border-slate-200 dark:border-slate-700 black:border-zinc-800">
                                Voltar
                            </button>
                    ...
                    {{-- dias --}}
                    @for ($day = 1; $day <= $daysInMonth; $day++) @php $events=$monthEvents[$day] ?? [];
                        $isToday=$day==12; @endphp <div data-month-day="{{ $day }}"
                        ...
                        <div class="flex items-center justify-between">
                            <span class="text-xs font-bold text-slate-800 dark:text-white black:text-zinc-100">{{ $day
                                }}</span>

                            @if (count($events))
                            <span class="text-[10px] font-bold text-slate-500 dark:text-slate-400 black:text-zinc-400">
                                {{ count($events) }}
                            </span>
                            @endif
                        </div>
    ...
    <div id="modal-new-commitment" class="hidden fixed inset-0 z-50">
        <div data-close-modal class="absolute inset-0 bg-black/40 dark:bg-black/70"></div>

        <div class="relative mx-auto mt-24 w-[92%] max-w-lg rounded-xl bg-white dark:bg-slate-800 black:bg-zinc-900
                border border-slate-200 dark:border-slate-700 black:border-zinc-800 shadow-xl p-4">
            <div class="flex items-center justify-between">
                <h3 class="font-bold text-slate-800 dark:text-white black:text-zinc-100">Novo Compromisso</h3>

                <button type="button" data-close-modal class="w-9 h-9 inline-flex items-center justify-center rounded-lg
                        text-slate-500 hover:text-slate-900 hover:bg-slate-100
                        dark:text-slate-400 dark:hover:text-white dark:hover:bg-slate-700
                        black:text-zinc-400 black:hover:text-zinc-100 black:hover:bg-zinc-800">
            ...
            <form id="form-new-commitment" class="mt-4 space-y-3">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                        <label
                            class="block text-xs font-bold text-slate-600 dark:text-slate-300 black:text-zinc-300 mb-1">Data</label>
                        <input name="date" type="date" class="w-full rounded-lg border border-slate-200 dark:border-slate-600 black:border-zinc-800 bg-white dark:bg-slate-700 black:bg-zinc-950
                               text-slate-700 dark:text-white black:text-zinc-100 px-3 py-2 text-sm" />
                    </div>

                    <div>
                        <label
                            class="block text-xs font-bold text-slate-600 dark:text-slate-300 black:text-zinc-300 mb-1">Hora</label>
                        <input name="time" type="time" class="w-full rounded-lg border border-slate-200 dark:border-slate-600 black:border-zinc-800 bg-white dark:bg-slate-700 black:bg-zinc-950
                               text-slate-700 dark:text-white black:text-zinc-100 px-3 py-2 text-sm" />
                    </div>
                </div>
                ...
                <div>
                    <label
                        class="block text-xs font-bold text-slate-600 dark:text-slate-300 black:text-zinc-300 mb-1">Título</label>
                    <input name="title" type="text" placeholder="Ex: Reunião com secretariado" class="w-full rounded-lg border border-slate-200 dark:border-slate-600 black:border-zinc-800 bg-white dark:bg-slate-700 black:bg-zinc-950
                           text-slate-700 dark:text-white black:text-zinc-100 px-3 py-2 text-sm" />
                </div>

                <div>
                    <label
                        class="block text-xs font-bold text-slate-600 dark:text-slate-300 black:text-zinc-300 mb-1">Local
                        (opcional)</label>
                    <input name="location" type="text" placeholder="Ex: Gabinete" class="w-full rounded-lg border border-slate-200 dark:border-slate-600 black:border-zinc-800 bg-white dark:bg-slate-700 black:bg-zinc-950
                           text-slate-700 dark:text-white black:text-zinc-100 px-3 py-2 text-sm" />
                </div>

                <div class="pt-2 flex items-center justify-end gap-2">
                    <button type="button" data-close-modal class="px-4 py-2 rounded-lg text-sm font-bold border border-slate-200
                           text-slate-700 hover:bg-slate-50
                           dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800
                           black:border-zinc-800 black:text-zinc-200 black:hover:bg-zinc-800">
                        Fechar
                    </button>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const WEEK_HDR = `
            <span class="text-slate-400 dark:text-slate-500 black:text-zinc-500 font-bold">D</span>
            <span class="text-slate-400 dark:text-slate-500 black:text-zinc-500 font-bold">S</span>
            <span class="text-slate-400 dark:text-slate-500 black:text-zinc-500 font-bold">T</span>
            <span class="text-slate-400 dark:text-slate-500 black:text-zinc-500 font-bold">Q</span>
            <span class="text-slate-400 dark:text-slate-500 black:text-zinc-500 font-bold">Q</span>
            <span class="text-slate-400 dark:text-slate-500 black:text-zinc-500 font-bold">S</span>
            <span class="text-slate-400 dark:text-slate-500 black:text-zinc-500 font-bold">S</span>
            `;
            ...
            function renderMini() {
                ...
                <button type="button" data-mini-day="${d}"
                    class="p-2 rounded-md hover:bg-indigo-50 dark:hover:bg-slate-700 black:hover:bg-zinc-800 ${active ? 'bg-indigo-600 text-white' : 'text-slate-600 dark:text-slate-400 black:text-zinc-400'}">
                    ${d}
                </button>
                `;
                }
            ...
            function renderDay() {
                ...
                if (!items.length) {
                    dayCards.innerHTML = `
                <div class="bg-white dark:bg-slate-800 black:bg-zinc-900 p-5 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 black:border-zinc-800">
                    <div class="text-slate-600 dark:text-slate-300 black:text-zinc-300 font-bold">Sem compromissos para este dia.</div>
                    <div class="text-sm text-slate-500 dark:text-slate-400 black:text-zinc-400 mt-1">Selecione outro dia no calendário.</div>
                </div>
                `;
                    return;
                }
                ...
                dayCards.innerHTML = items.map(ev => {
                    ...
                    return `
                <div class="bg-white dark:bg-slate-800 black:bg-zinc-900 p-5 rounded-xl shadow-sm border-l-4 ${border} border-y border-r border-slate-200 dark:border-slate-700 black:border-zinc-800 flex items-center justify-between group hover:shadow-md transition-shadow">
                    <div class="flex items-center gap-6">
                    <div class="text-center min-w-[60px]">
                        <span class="block text-lg font-bold text-slate-800 dark:text-white black:text-zinc-100">${start}</span>
                        ${end ? `<span class="text-xs text-slate-500 dark:text-slate-400 black:text-zinc-500 uppercase">${end}</span>` : ``}
                    </div>
                    <div>
                        <h4 class="font-bold text-slate-800 dark:text-white black:text-zinc-100">${title}</h4>
                        ${loc ? `<p class="text-sm text-slate-500 dark:text-slate-400 black:text-zinc-400 mt-1">📍 ${loc}</p>` : ``}
                    </div>
                    </div>
                    <div class="flex items-center gap-3">
                    ${tag ? `<span class="px-3 py-1 ${tagCls} rounded-full text-xs font-bold">${tag}</span>` : ``}
                    <button type="button" class="text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-white black:text-zinc-500 black:hover:text-zinc-200 transition-colors" aria-label="Mais opções">⋮</button>
                    </div>
                </div>
                `;
                }).join('');
            ...
            function renderMonth() {
                ...
                <div data-month-day="${d}"
                    class="min-h-[110px] rounded-xl border border-slate-200 dark:border-slate-700 black:border-zinc-800 bg-white dark:bg-slate-800 black:bg-zinc-900 p-2 flex flex-col gap-2 cursor-pointer ${ring}">
                    <div class="flex items-center justify-between">
                    <span class="text-xs font-bold text-slate-800 dark:text-white black:text-zinc-100">${d}</span>
                    ${items.length ? `<span class="text-[10px] font-bold text-slate-500 dark:text-slate-400 black:text-zinc-400">${items.length}</span>` : ``}
                    </div>
