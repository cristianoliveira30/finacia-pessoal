            ...
            <div class="flex items-center gap-2">
                ...
                <div class="inline-flex rounded-xl m-0 border border-slate-200 bg-slate-50 overflow-hidden dark:border-slate-700 dark:bg-slate-900/60">
                    <button type="button" data-card-view-toggle="{{ $id }}" data-view="chart"
                        class="text-xs px-3 py-2 font-medium bg-white text-slate-600 hover:bg-slate-50 hover:text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-300 dark:bg-slate-800/80 dark:text-slate-200 dark:hover:bg-slate-700 dark:hover:text-white dark:focus:ring-sky-500/40">
                        <x-bi-graph-up />
                    </button>
                ...
                {{-- Botão Collapse --}}
                <button type="button" data-accordion-target="#{{ $accordionBodyId }}" aria-expanded="false" aria-controls="{{ $accordionBodyId }}"
                    class="text-xs px-3 py-2 font-medium bg-white text-slate-600 border border-slate-200 rounded-lg hover:bg-slate-50 hover:text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-300 dark:bg-slate-800/80 dark:text-slate-200 dark:border-slate-600 dark:hover:bg-slate-700 dark:hover:text-white dark:focus:ring-sky-500/40">
                    <x-bi-caret-down-fill class="w-3 h-3 shrink-0 transition-transform duration-200" data-accordion-icon />
                </button>
            </div>
        </div>
        ...
        <div id="{{ $accordionBodyId }}" class="hidden" aria-labelledby="{{ $id }}-collapse-heading">
            ...
            <div>
                <div data-card-section="chart">
                    @switch($chartType)
                        @case('area') <x-cards.graph.area-chart :data="$chart" /> @break
                        @case('pie') <x-cards.graph.pie-chart :data="$chart" /> @break
                        @case('column') <x-cards.graph.column-chart :data="$chart" /> @break
                        @case('bar') <x-cards.graph.bar-chart :data="$chart" /> @break
                        @case('radial') <x-cards.graph.radial-chart :data="$chart" /> @break
                        @default
                    @endswitch
                </div>
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 p-3 border-t border-slate-100 dark:border-slate-700/80">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button type="button" id="{{ $id }}-btn-refresh" onclick="Refresh('{{ $id }}')" data-tooltip-target="{{ $tooltipId }}" data-tooltip-placement="top" class="text-xs px-3 py-2 font-medium bg-sky-500 text-white border border-slate-200 rounded-lg hover:bg-sky-600 hover:text-white focus:outline-none focus:ring-2 focus:ring-sky-300 dark:bg-sky-800/80 dark:border-slate-600 dark:hover:bg-sky-700 dark:hover:text-white dark:focus:ring-sky-500/40">
                        <x-bi-arrow-clockwise />
                    </button>
                    <div id="{{ $tooltipId }}" role="tooltip" class="absolute z-20 invisible inline-block px-3 py-2 text-xs font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-700">
                        Atualizar indicador <div class="tooltip-arrow" data-popper-arrow></div>
                    </div>

                    {{-- Botões Toggle Sections --}}
                    <button type="button" data-collapse-toggle="{{ $aiSectionId }}" aria-controls="{{ $aiSectionId }}" aria-expanded="false" class="text-xs px-3 py-2 font-medium bg-white text-slate-600 border border-slate-200 rounded-lg hover:bg-slate-50 hover:text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-300 dark:bg-slate-800/80 dark:text-slate-200 dark:border-slate-600 dark:hover:bg-slate-700 dark:hover:text-white dark:focus:ring-sky-500/40">
                        <x-bi-openai />
                    </button>
                    <button type="button" data-collapse-toggle="{{ $filterSectionId }}" aria-controls="{{ $filterSectionId }}" aria-expanded="false" class="text-xs px-3 py-2 font-medium bg-white text-slate-600 border border-slate-200 rounded-lg hover:bg-slate-50 hover:text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-300 dark:bg-slate-800/80 dark:text-slate-200 dark:border-slate-600 dark:hover:bg-slate-700 dark:hover:text-white dark:focus:ring-sky-500/40">
                        <x-bi-funnel-fill />
                    </button>

                    {{-- Dropdown Download --}}
                    <div class="relative">
                        <button id="{{ $downloadButtonId }}" data-dropdown-toggle="{{ $downloadDropId }}" type="button" class="inline-flex items-center gap-1.5 px-3 py-2 text-xs md:text-sm font-medium rounded-lg border justify-center bg-slate-50 text-slate-600 border-slate-200 hover:bg-slate-100 hover:text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-300 dark:bg-slate-800/80 dark:text-slate-300 dark:border-slate-600 dark:hover:bg-slate-700 dark:hover:text-slate-50 dark:focus:ring-sky-500/40">
                            <x-bi-download class="w-4 h-4" />
                        </button>
                        <div id="{{ $downloadDropId }}" class="z-20 hidden mt-2 bg-white divide-y divide-slate-100 rounded-lg shadow-lg border border-slate-100 dark:bg-slate-800 dark:divide-slate-700 dark:border-slate-700">
                            <ul class="py-2 text-sm text-slate-700 dark:text-slate-200" aria-labelledby="{{ $downloadButtonId }}">
                                <li>
                                    <a href="#" id="{{ $id }}-btn-download-pdf" class="flex items-center gap-2 px-4 py-2 hover:bg-slate-50 dark:hover:bg-slate-700/70 dark:hover:text-white">
                                        <x-bi-filetype-pdf class="w-4 h-4 shrink-0" />
                                    </a>
                                </li>
                                <li>
                                    <a href="#" id="{{ $id }}-btn-download-xlsx" class="flex items-center gap-2 px-4 py-2 hover:bg-slate-50 dark:hover:bg-slate-700/70 dark:hover:text-white">
                                        <x-bi-filetype-xlsx class="w-4 h-4 shrink-0" />
                                    </a>
                                </li>
                                <li>
                                    <a href="#" id="{{ $id }}-btn-download-csv" class="flex items-center gap-2 px-4 py-2 hover:bg-slate-50 dark:hover:bg-slate-700/70 dark:hover:text-white">
    ...
    @push('scripts')
        <script>
            ...
            window.setupCardDownloads = function(cardId, data) {
                ...
                const actions = [
                    { id: 'pdf', fn: window.baixarPDF },
                    { id: 'csv', fn: window.baixarCSV },
                    { id: 'xlsx', fn: window.baixarXLSX }
                ];
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (window.setupCardDownloads) window.setupCardDownloads('{{ $id }}', @json($chart));
        });
    </script>
@endpush