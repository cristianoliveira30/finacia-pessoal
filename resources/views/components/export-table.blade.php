...
<div class="w-full overflow-x-auto">
    <table id="{{ $tableId }}">
                    ...
                    <th{!! $thAttrs !!}>
                        <div class="flex flex-col gap-1 min-w-[120px]">
                            <div class="flex items-center justify-between group">
                                {{-- AUMENTO DE CONTRASTE: text-gray-700 | black:text-zinc-300 --}}
                                <span class="mr-2 text-gray-700 dark:text-gray-200 black:text-zinc-700 font-semibold">{{ $label }}</span>
                                
                                <button type="button" 
                                        class="column-filter-toggle text-gray-400 hover:text-blue-600 focus:outline-none p-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 black:hover:bg-zinc-800 transition-colors"
                            ...
                            <div class="column-filter-container hidden mt-1" id="filter-container-{{ $tableId }}-{{ $index }}">
                                {{-- INPUT FILTRO COLUNA: black:bg-zinc-900 black:border-zinc-700 black:text-zinc-100 --}}
                                <input type="{{ $inputType }}" 
                                       class="column-filter-input w-full text-xs px-2 py-1 border border-gray-300 rounded focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white black:bg-zinc-900 black:border-zinc-700 black:text-zinc-100 black:placeholder-zinc-500"
                                       placeholder="Buscar..."
                                       data-col-index="{{ $index }}"
                                       step="any">
                            </div>
        ...
        <tbody>
            @foreach ($rows as $row)
                {{-- HOVER ROW: black:hover:bg-zinc-800/50 --}}
                <tr class="hover:bg-neutral-secondary-soft dark:hover:bg-slate-800/50 black:hover:bg-zinc-800/50 cursor-pointer transition-all duration-500">
                    @foreach ($columns as $column)
                        ...
                        {{-- TEXTO DA CÉLULA: black:text-zinc-200 / black:text-zinc-400 --}}
                        <td class="{{ $isFirst ? 'font-semibold text-gray-800 dark:text-gray-100 black:text-zinc-200 whitespace-nowrap' : 'text-gray-600 dark:text-gray-300 black:text-zinc-400 whitespace-nowrap' }}">
                            {{ $value }}
                        </td>
                    @endforeach
                </tr>
    ...
    @push('scripts')
        <script>
            ...
            window.toggleFilterTable = function(containerId, buttonId) {
                ...
                if (isVisible) {
                    button.classList.add('bg-gray-100', 'dark:bg-gray-600', 'black:bg-zinc-700', 'ring-2', 'ring-gray-200', 'dark:ring-gray-500', 'black:ring-zinc-600');
                    button.classList.remove('bg-white', 'dark:bg-gray-700', 'black:bg-zinc-800');
                } else {
                    button.classList.remove('bg-gray-100', 'dark:bg-gray-600', 'black:bg-zinc-700', 'ring-2', 'ring-gray-200', 'dark:ring-gray-500', 'black:ring-zinc-600');
                    button.classList.add('bg-white', 'dark:bg-gray-700', 'black:bg-zinc-800');
                }
            };
            ...
            window.initExportReportTable = function(tableId, extraOptions = {}, rawData = {}) {
                    ...
                    extraOptions.datatable || {}, {
                        ...
                        template: (options, dom) =>
                            ...
                            (options.searchable ?
                                ...
                                "<input class='" + options.classes.input + " w-full sm:w-auto dark:bg-gray-700 dark:border-gray-600 dark:text-white black:bg-zinc-800 black:border-zinc-700 black:text-zinc-200 black:placeholder-zinc-500' placeholder='" + options.labels.placeholder + "' type='search' title='" + options.labels.searchTitle + "'" + (dom.id ? " aria-controls='" + dom.id + "'" : "") + ">" +
                                "<button id='" + filterBtnId + "' type='button' class='flex-shrink-0 text-body bg-neutral-secondary-medium box-border border border-default-medium hover:bg-neutral-tertiary-medium hover:text-heading focus:ring-2 focus:ring-neutral-tertiary shadow-xs font-medium leading-5 rounded-base text-sm px-3 py-2 focus:outline-none transition-colors duration-200 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200 black:bg-zinc-800 black:border-zinc-700 black:text-zinc-200 black:hover:bg-zinc-700'>" +
                            ...
                            "</div>" +
                            "<form action='" + filterAction + "' method='GET' id='" + filterContainerId + "' class='hidden w-full bg-gray-50 dark:bg-gray-800 black:bg-zinc-900 border border-gray-200 dark:border-gray-700 black:border-zinc-700 rounded-md p-4 mb-3 transition-all duration-300'>" +
                            "<div class='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4'>" +
                            "<div><label class='block text-xs font-medium text-gray-700 dark:text-gray-300 black:text-zinc-400 mb-1'>Data Início</label><input type='date' name='data_inicio' class='w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white black:bg-zinc-800 black:border-zinc-600 black:text-zinc-100'></div>" +
                            "<div><label class='block text-xs font-medium text-gray-700 dark:text-gray-300 black:text-zinc-400 mb-1'>Data Fim</label><input type='date' name='data_fim' class='w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white black:bg-zinc-800 black:border-zinc-600 black:text-zinc-100'></div>" +
                            "<div><label class='block text-xs font-medium text-gray-700 dark:text-gray-300 black:text-zinc-400 mb-1'>Status</label><select name='status' class='w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white black:bg-zinc-800 black:border-zinc-600 black:text-zinc-100'><option value=''>Todos</option><option value='ativo'>Ativo</option><option value='inativo'>Inativo</option></select></div>" +
                            "<div class='flex items-end'><button type='submit' class='w-full dark:bg-blue-900 dark:hover:bg-blue-700 black:bg-zinc-700 black:hover:bg-zinc-600 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline text-xs'>Aplicar Filtros</button></div>" +
                            "</div>" +
                            "</form>" + 
                ...
                function moveMatchesToTop(colIndex, term) {
                    ...
                    matches.forEach(r => {
                        r.classList.add('bg-blue-50', 'dark:bg-blue-900/20', 'black:bg-blue-900/20'); 
                        fragment.appendChild(r);
                    });
                    others.forEach(r => {
                        r.classList.remove('bg-blue-50', 'dark:bg-blue-900/20', 'black:bg-blue-900/20');
                        fragment.appendChild(r);
                    });
